...
Эта статья создана потому, что я постоянно забываю как обращаться к аругментам функций в разных сложных случаях. Для упрощения ситуации речь пойдет про python 3.x.
...
- обязательные аргументы сопоставляются слева направо с любыми переданными позиционными аргументами _def func{имя)_

- стандартные аргументы, которые будут присвоены аргументу, если в вызове функции аргумент не передавался _def func{имя=значение)_
...
7. Присваивание именам аргументов переданные для них объекты

При этом учитывается то, какие из аргументов могут быть присвоены только по ключевым словам. Аргументы с передачей только по ключевым словам должны указываться после \*, но не могут указываться после или \*\*аргументы. Также, конструкция \*\* не может появляться в списке аргументов сама посебе.
